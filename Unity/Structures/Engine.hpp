...
namespace Unity
{
	struct Vector2
	{
		...
		const char* ToString() const
		{
			...
			sprintf_s(buffer, "Vector2(%.2f, %.2f)", x, y);
			return buffer;
		}
	...
	struct Vector3
	{
		...
		static Vector3 Sub(Vector3 to, Vector3 from) {
			Vector3 f;
			f.x = to.x - from.x;
			f.y = to.y - from.y;
			f.z = to.z - from.z;
		...
		static Vector3 Add(Vector3 to, Vector3 from) {
			Vector3 f;
			f.x = to.x + from.x;
			f.y = to.y + from.y;
			f.z = to.z + from.z;
		...
		{
			static char buffer[128];
			sprintf_s(buffer, "Vector3(%.2f, %.2f, %.2f)", x, y, z);
			return buffer;
		}
